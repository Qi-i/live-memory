...
try {
  ...
  const iconSvg = await readFile(new URL("../public/icon.svg", import.meta.url), "utf8");
  const logoSvg = await readFile(new URL("../public/live-memory-logo.svg", import.meta.url), "utf8");
  const brandLockupSvg = await readFile(new URL("../public/brand-lockup.svg", import.meta.url), "utf8");
  ...
  assert.match(brand, /现场记 · CONCERT ARCHIVE/);
  assert.match(brand, /live-memory-spotlight-gradient/);
  assert.match(brand, /rotate\(-13 202 281\)/);
  assert.match(brandCss, /experience-brand-mark/);
  assert.match(iconSvg, /#0A4A40/);
  assert.match(iconSvg, /#F9EACB/);
  assert.match(logoSvg, /Live Memory/);
  assert.match(brandLockupSvg, /CONCERT ARCHIVE · 现场记/);
  assert.doesNotMatch(brand, /#315ED8|#65E2CE|two.*track/i);
  assert.match(archive, /archive-brand-kicker/);
  assert.match(shareStudio, /function drawBrandMark/);
