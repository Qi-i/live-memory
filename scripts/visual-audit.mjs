...
const baseUrl = process.env.VISUAL_AUDIT_URL || "http://127.0.0.1:4173/live-memory/";
...
async function addRecord({ title, artist, city, venue, date, price, colors }) {
  ...
  await page.getByLabel("演出名称").fill(title);
  await page.getByLabel("艺人 / 阵容").fill(artist);
  await page.getByLabel("日期").fill(date);
  await page.getByLabel("城市").fill(city);
  await page.getByLabel("场馆").fill(venue);
  await page.getByLabel("票价").fill(String(price));
  await page.locator(".media-upload-grid-v2 label").first().locator("input[type=file]").setInputFiles({
    name: `${title}.svg`,
...
try {
  ...
  await page.getByRole("button", { name: "临时进入" }).click();
  await page.locator(".experience-shell").waitFor({ state: "visible" });
  ...
  await page.getByRole("button", { name: "分享画布" }).click();
  ...
  await page.getByRole("button", { name: "退出分享" }).click();

  await page.setViewportSize({ width: 390, height: 844 });
  await page.getByTitle("海报").click();
  await page.screenshot({ path: `${outputDir}/05-archive-mobile.png`, fullPage: true });
  await page.locator(".experience-mobile-nav").getByRole("button", { name: "设置" }).click();
