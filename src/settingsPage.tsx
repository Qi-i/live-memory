    ...
    <section className="settings-page-v2">
      ...
      <div className="settings-layout-v2">
        ...
        <aside className="settings-side-v2">
          ...
          <section className="settings-module compact-settings-v2">
            <ModuleHeader eyebrow="地图" title="城市与场馆足迹" description="中国固定足迹图无需密钥；启用在线地图时按下方指引申请浏览器端 API。" />
            <label title="中国固定足迹图无需 API；高德或百度在线地图需要各自的浏览器端密钥。">地图来源<select value={draft.map.provider} onChange={(event) => setDraft({ ...draft, map: { ...draft.map, provider: event.target.value as AppSettings["map"]["provider"] } })}><option value="none">中国固定足迹图（无需 API）</option><option value="amap">高德地图</option><option value="baidu">百度地图</option></select></label>
